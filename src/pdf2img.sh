...
quality=$(validate_number "$quality" "请输入质量 (默认: 85): ")
...
# 调试输出
echo "计算的 resize_value: $resize_value"

# 构建 convert 命令
convert_command="magick \"$input_pdf\" -density \"$density\" -quality \"$quality\" -resize \"$resize_value\"  \"$output_dir/output_%03d.$output_format\""

# 打印要执行的命令
echo "要执行的命令: $convert_command"

# 使用 ImageMagick 的 convert 命令将 PDF 转换为图片
